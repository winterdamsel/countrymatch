 ...
 */
 var frcountries = {
	 "MX": {
		friends: {
			"IN" : "Colourful festivals - Spicy Food - Crunchy fried things with sauce - Spices-mix based sauce - Flat flour-based food item - Enormous parties - Casseroles during parties - Dancing is a must at parties - Folded flour-based flat item with stuffing inside",
			"GR" : "Sweet fried dough - Greet with a kiss - Enormous parties - Drinking is ok"
	 }},
	"FI": {
		friends: {
			"MX": "Blood as the main ingredient",
			"IN": "Use of cardamom"
	}}
 };
...
(function (factory) {
    ...
    Mapael.prototype = {
		...
		getList: function (id, elem){
			console.log(" Getting a list "+id);
			$.each(frcountries, function(i, value){
				if(i == id){	
				var inpt ="";
					$.each(value, function(y, con){
						$.each(con, function (z, finc){
						console.log(z+"--"+finc);
						inpt += "<p>"+z+"</p><p>"+finc+"</p>";
						$("#matches").html(inpt);
        ...
        setTooltip: function (elem) {
            ...
            $(elem.node).on("mouseover." + pluginName, function (e) {
               ...
               console.log("We're clicking!"+elem.tooltip.pairs);
			   self.getList(elem.tooltip.content(elem), elem);
            });
        },
